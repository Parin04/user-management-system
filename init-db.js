...
async function initializeDatabase() {
    ...
    try {
        ...
        // ลบข้อมูลตัวอย่างเก่าก่อน (ถ้ามี)
        console.log('🔄 ตรวจสอบและอัปเดตข้อมูลตัวอย่าง...');
        
        // Get admin user ID
        const adminUser = await client.query('SELECT id FROM users WHERE username = $1', ['admin']);
        const adminId = adminUser.rows[0].id;

        // ลบข้อมูลตัวอย่างเก่าที่สร้างโดย admin
        await client.query('DELETE FROM customers WHERE created_by = $1', [adminId]);
        await client.query('DELETE FROM employees WHERE created_by = $1', [adminId]);
        
        console.log('🔄 กำลังสร้างข้อมูลตัวอย่างใหม่สำหรับ Demo...');
        ...
        console.log('✅ สร้างข้อมูลตัวอย่างใหม่สำเร็จ');
        console.log('📊 ข้อมูล Demo ที่อัปเดต:');
